...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let indents: Vec<_> = input_data
        .fields
        .into_iter()
        .map(|f| {
            let f_name = format_ident!("{}", f.ident.unwrap());
        ...
        .collect();
    let bs_attr: Vec<_> = indents
        .iter()
        .map(|(f_name, f_type)| {
            quote! { #f_name: Option<#f_type> }
        })
        .collect();
    let bf_attr: Vec<_> = indents
        .iter()
        .map(|(f_name, _)| {
            quote! { #f_name: None }
        })
        .collect();
    let funcs: Vec<_> = indents
        .iter()
        .map(|(f_name, f_type)| {
            quote! {
        ...
        .collect();
    let build: Vec<_> = indents.iter()
        .map(|(f_name, _)| {
            let f_string = format!("{f_name}");
            quote! {
